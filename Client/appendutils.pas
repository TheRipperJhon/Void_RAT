...
function GetOption(VOPT: String; Num: Byte): Byte;
...
Begin
 ...
 try
   ...
   Full:=LeftStr(Full, Length(Full)-Length(sLineBreak));
   CloseFile(F);
 except
   Full:='';
 end;
...
Begin
 X:=VOPT;
 For J:=1 to Num-1 do Delete(X, 1, 1);
 Result:=StrToInt(X[1]);
end;
